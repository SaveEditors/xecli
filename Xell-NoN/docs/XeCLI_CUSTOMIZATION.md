...
- The staged dump payload shows a stable `Waiting for XeCLI connection` screen instead of dropping into the normal file/TFTP search loop.
- The screen now carries a XeCLI ASCII wordmark above the live dump status.
...
- When the dump finishes, XeLL switches to a persistent success screen instead of falling back into the idle TFTP loop.
