...
public class XellCommandSettings : ConnectionSettings
{
	...
	[Description("Force the direct XeLL reboot path instead of the XellLaunch shortcut when available.")]
	public bool ForceXell { get; init; }

	[CommandOption("--yes")]
	[Description("Skip the interactive XeLL launch confirmation prompt.")]
	public bool Yes { get; init; }

	[CommandOption("--launcher <PATH>")]
	[Description("Remote XEX path to use as the XeLL launch helper, for example Hdd1:\\XellLaunch\\default.xex.")]
	public string? LauncherPath { get; init; }

	[CommandOption("--stage-launcher <FILE>")]
	[Description("Upload a local XeLL launch helper XEX before booting. Defaults to Hdd1:\\XellLaunch\\default.xex when --launcher is omitted.")]
	public string? StageLauncherPath { get; init; }

	[CommandOption("--stage-xell-bin <FILE>")]
	[Description("Upload a local XeLL binary as xell.bin beside the launch helper before booting.")]
	public string? StageXellBinPath { get; init; }

	[CommandOption("--quickboot")]
	[Description("Build, upload, and launch a QuickBoot XeLL launcher, and also install a dashboard shortcut when possible.")]
	public bool QuickBoot { get; init; }

	[CommandOption("--quickboot-target <MODE>")]
	[Description("QuickBoot target: launcher (default) or flash.")]
	public string? QuickBootTarget { get; init; }

	[CommandOption("--allow-usb")]
	[Description("Allow XeLL launch even when removable USB storage is attached. Disabled by default because some consoles hang at Fat mount uda0.")]
