    ...
    public override async Task<int> ExecuteAsync(CommandContext context, Settings settings)
    {
        ...
        if (settings.Json)
        {
            CliOutput.EmitJson(new {
                Source = FatmanCommandHelpers.DescribeSource(source),
                Open = FatmanCommandHelpers.DescribeOpenOptions(openOptions),
                OutputDirectory = FatmanCommandHelpers.DescribePathLeaf(outputDirectory),
                Dumped = selected.Select(partition => new {
                    ...
                    partition.Offset,
                    partition.Length,
                    File = $"{partition.Index:D2}-{FatmanCommandHelpers.SanitizeName(partition.Name)}.bin"
                })
            });
        }
        else
...
public sealed class FatmanPutCommand : AsyncCommand<FatmanPutCommand.Settings>
{
    public sealed class Settings : FatmanPathExportSettings
    {
        ...
        [LocalizedDescription("Host file to write into the selected FATX source.")]
...
internal static class FatmanCommandHelpers
{
    ...
    public static FatxOpenOptions CreateOpenOptions(FatmanSourceInfo source, FatmanImageSettings settings, bool readOnly = true)
    {
        ...
        if (length.HasValue)
        {
            ...
            if (length.Value > fileLength - offset!.Value)
    ...
    public static object DescribeSource(FatmanSourceInfo source)
    {
        return new
        {
            DisplayName = DescribeSourceDisplayName(source),
            source.Length,
    ...
    private static string DescribeSourceDisplayName(FatmanSourceInfo source)
    {
        if (source.IsPhysicalDisk)
        {
            if (source.Disk is not null)
                return $"{source.Disk.FriendlyName} (Disk {source.Disk.Number})";

            return "Physical disk";
        }

        string displayName = Path.GetFileName(source.SourcePath);
    ...
    public static string DescribePathLeaf(string path)
    {
