...
public sealed class FatmanMetadataRestoreCommand : AsyncCommand<FatmanMetadataRestoreCommand.Settings>
{
    public sealed class Settings : FatmanImageSettings
    {
        ...
        [LocalizedDescription("Show the restore plan without opening the destination for write access or applying changes.")]
    ...
    public override async Task<int> ExecuteAsync(CommandContext context, Settings settings)
    {
        ...
        IReadOnlyList<RestorePlanItem> plan = BuildRestorePlan(manifest, manifestPath);
        ...
        if (dryRun)
        {
            ...
            {
                AnsiConsole.Write(new Rule("[bold deepskyblue1]Fatman Metadata Restore Preview[/]").RuleStyle("grey"));
                AnsiConsole.MarkupLine($"[grey]Source:[/] [white]{Markup.Escape(source.DisplayName)}[/]");
                AnsiConsole.MarkupLine($"[grey]Manifest:[/] [white]{Markup.Escape(Path.GetFileName(manifestPath))}[/]");
                AnsiConsole.MarkupLine($"[grey]Open:[/] [grey]mode:[/] read-only, [grey]offset:[/] {(openOptions.PartitionOffset.HasValue ? $"[cyan1]0x{openOptions.PartitionOffset.Value:X}[/]" : "[grey70]-[/]")}, [grey]length:[/] {(openOptions.PartitionLength.HasValue ? FatmanCommandHelpers.FormatBytes(openOptions.PartitionLength.Value) : "[grey70]-[/]")}");
                ...
                foreach (RestorePlanItem item in plan)
                {
                    table.AddRow(
                        $"[springgreen3_1]{Markup.Escape(item.Name)}[/]",
                        $"[deepskyblue1]0x{item.Offset:X}[/]",
                        FatmanCommandHelpers.FormatBytes(item.Length),
                        $"[white]{Markup.Escape(item.FileName)}[/]");
                }
        ...
        if (!settings.AutoConfirm)
        {
            if (!ConfirmationHelpers.TryConfirm(
                    "Fatman metadata restore",
                    $"This will overwrite low-level metadata on {source.DisplayName}. Continue?",
    ...
    private static IReadOnlyList<RestorePlanItem> BuildRestorePlan(FatmanMetadataManifest manifest, string manifestPath)
    {
        string manifestDirectory = Path.GetDirectoryName(manifestPath) ?? throw new InvalidOperationException("The metadata manifest path is invalid.");
        return manifest.Regions.Select(region =>
        {
            string regionPath = Path.Combine(manifestDirectory, region.FileName);
            if (!File.Exists(regionPath))
                throw new FileNotFoundException("Metadata region file not found.", regionPath);

            return new RestorePlanItem(region.Name, region.Offset, region.Length, region.FileName);
        }).ToArray();
    }
    ...
    private sealed record RestorePlanItem(string Name, long Offset, long Length, string FileName);
}
