...
public sealed class FatmanCheckCommand : AsyncCommand<FatmanCheckCommand.Settings>
{
    ...
    public override Task<int> ExecuteAsync(CommandContext context, Settings settings)
        => FatmanRepairCommand.RunAsync(settings, applyRepairs: false, json: settings.Json);
}
...
public sealed class FatmanRepairCommand : AsyncCommand<FatmanRepairCommand.Settings>
{
    ...
    public override Task<int> ExecuteAsync(CommandContext context, Settings settings)
        => RunAsync(settings, applyRepairs: true, json: settings.Json);

    internal static async Task<int> RunAsync(FatmanPartitionSettings settings, bool applyRepairs, bool json)
    {
        ...
        if (applyRepairs && !autoConfirm)
        {
            if (!ConfirmationHelpers.TryConfirm(
                    "Fatman repair",
                    $"This will modify FATX chain metadata in {partition.Name}. Continue?",
                    autoConfirm,
                    emitWarning: false))
            {
