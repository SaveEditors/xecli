...
public sealed class GhidraConfigCommand : Command<GhidraConfigCommand.Settings> {
    ...
    public override int Execute(CommandContext context, Settings settings) {
        ...
        if (!CliConfig.TryLoad(out CliConfig config)) {
            AnsiConsole.MarkupLine("[red]Config file is present but could not be parsed. Fix or remove config.json, then run rgh ghidra config again.[/]");
            return 1;
        }
        if (settings.Clear) {
...
public sealed class GhidraAnalyzeCommand : AsyncCommand<GhidraAnalyzeCommand.Settings> {
    public sealed class Settings : CommandSettings {
        ...
        [LocalizedDescription("Delete a project created by this run after analysis completes.")]
    ...
    public override async Task<int> ExecuteAsync(CommandContext context, Settings settings) {
        ...
        if (!File.Exists(analyzePath)) {
            AnsiConsole.MarkupLine("[red]Ghidra analyzeHeadless tool is missing.[/]");
            return 1;
        }
...
public sealed class GhidraDecompileCommand : AsyncCommand<GhidraDecompileCommand.Settings> {
    ...
    public override async Task<int> ExecuteAsync(CommandContext context, Settings settings) {
        ...
        if (settings.MaxFunctions.HasValue && settings.MaxFunctions.Value <= 0) {
            OperationFeedback.WriteFailure("Invalid decompile limit", "--max must be greater than zero.");
            return 1;
        }

        if (settings.FunctionTimeoutSeconds.HasValue && settings.FunctionTimeoutSeconds.Value <= 0) {
            OperationFeedback.WriteFailure("Invalid decompile timeout", "--func-timeout must be greater than zero.");
            return 1;
        }
...
public sealed class GhidraExportSymbolsCommand : AsyncCommand<GhidraExportSymbolsCommand.Settings> {
    public sealed class Settings : CommandSettings {
        [CommandOption("--in <FILE>")]
        [LocalizedDescription("Input file to analyze (XEX).")]
        public string? Input { get; init; }
    ...
    public override async Task<int> ExecuteAsync(CommandContext context, Settings settings) {
        ...
        string scriptRoot = settings.ScriptPath ?? Path.Combine(AppContext.BaseDirectory, "ghidra_scripts");
        string scriptFile = Path.Combine(scriptRoot, "ExportSymbols.java");
        if (!File.Exists(scriptFile)) {
            AnsiConsole.MarkupLine($"[red]Export script not found at[/] {Markup.Escape(scriptFile)}");
            return 1;
        }
        ...
        args.Add(scriptRoot);
        args.Add("-postScript");
        args.Add("ExportSymbols.java");
        args.Add(outputPath);
        args.Add(Path.GetFileNameWithoutExtension(inputPath));
...
public sealed class GhidraVerifyCommand : Command<GhidraVerifyCommand.Settings> {
    ...
    public override int Execute(CommandContext context, Settings settings) {
        ReverseEngineeringNoticeHelpers.WriteGhidraNotice(settings.Json);
        if (string.IsNullOrWhiteSpace(settings.Directory)) {
            if (WriteJsonError(settings.Json, "Ghidra verify validation failed", "--dir is required.", "GHIDRA_VERIFY_VALIDATION_FAILED"))
                return 1;
        ...
        if (!Directory.Exists(settings.Directory)) {
            if (WriteJsonError(settings.Json, "Ghidra verify validation failed", "Directory not found.", "GHIDRA_VERIFY_VALIDATION_FAILED"))
                return 1;
        ...
        try {
            ...
            if (WriteJsonError(settings.Json, "Ghidra verify validation failed", $"Invalid regex: {ex.Message}", "GHIDRA_VERIFY_VALIDATION_FAILED"))
                return 1;
...
internal static class GhidraInputHelpers {
    public static async Task<string?> ResolveRunningFtpPathAsync(bool quiet = false) {
        try {
    ...
    public static async Task<string?> DownloadViaFtpAsync(string ftpPath, bool quiet = false) {
...
internal static class GhidraLoaderHelpers {
    ...
    public static bool HasXexLoader(string? ghidraHome) {
        ...
        if (GhidraInstallHelpers.FindInstalledXexLoaderRoots(ghidraHome).Count > 0)
            return true;
        return CheckUserExtensions();
    }
