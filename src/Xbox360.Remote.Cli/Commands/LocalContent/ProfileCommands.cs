...
internal static class ProfileCommandHelpers
{
	public static int ExecuteWithProfile(string inputPath, Func<string, ProfilePackage, int> action)
	{
		...
		{
			return LocalContentHelpers.Fail("Unable to open profile package.");
		}
		finally
		{
			profile?.Dispose();
	...
	public static int ExecuteMutationWithProfile(
		string inputPath,
		string? keyVaultPath,
		Func<string, ProfilePackage, int> action)
	{
		return ExecuteWithProfile(inputPath, (fullPath, profile) =>
			ConSigningKeyHelpers.Execute(profile.SignatureType, keyVaultPath, () => action(fullPath, profile)));
	}
...
{
	internal sealed record ProfileTitleRow(
		uint TitleId,
		string TitleName,
		string ResolvedTitleName,
	...
	public sealed class Settings : CommandSettings
	{
		...
		[CommandOption("--csv")]
		[LocalizedDescription("Output CSV.")]
	...
	public override int Execute(CommandContext context, Settings settings)
	{
		return ProfileCommandHelpers.ExecuteWithProfile(settings.PackagePath, (_, profile) =>
		{
			...
			IReadOnlyList<ProfileTitleRow> titles = BuildTitleRows(profile.ReadTitles(), profile.HasTitleDataFile);
			...
			if (settings.Csv)
			{
				AnsiConsole.Console.Profile.Out.Writer.Write(RenderCsv(titles));
				return 0;
			}
	...
	internal static IReadOnlyList<ProfileTitleRow> BuildTitleRows(
		IReadOnlyList<ProfileTitleInfo> titles,
		Func<uint, bool> hasTitleData)
	{
		return titles
			.Select(title =>
			{
				string resolvedName = ResolveTitleName(title.TitleId, title.TitleName);
				return new ProfileTitleRow(
					title.TitleId,
					title.TitleName,
					resolvedName,
					title.AchievementsEarned,
	...
	internal static string RenderCsv(IReadOnlyList<ProfileTitleRow> titles)
	{
		StringBuilder builder = new StringBuilder();
		builder.AppendLine("TitleId,TitleName,ResolvedTitleName,AchievementsEarned,AchievementsPossible,CreditEarned,CreditPossible,LastLoadedUtc,HasTitleData");
		foreach (ProfileTitleRow title in titles)
		{
			builder.Append(Csv(LocalContentHelpers.FormatUInt32(title.TitleId))).Append(',');
			builder.Append(Csv(title.TitleName)).Append(',');
			builder.Append(Csv(title.ResolvedTitleName)).Append(',');
			builder.Append(Csv(title.AchievementsEarned.ToString(CultureInfo.InvariantCulture))).Append(',');
			builder.Append(Csv(title.AchievementsPossible.ToString(CultureInfo.InvariantCulture))).Append(',');
			builder.Append(Csv(title.CreditEarned.ToString(CultureInfo.InvariantCulture))).Append(',');
			builder.Append(Csv(title.CreditPossible.ToString(CultureInfo.InvariantCulture))).Append(',');
			builder.Append(Csv(title.LastLoadedUtc?.ToUniversalTime().ToString("O", CultureInfo.InvariantCulture) ?? string.Empty)).Append(',');
			builder.AppendLine(Csv(title.HasTitleData ? "true" : "false"));
		}

		return builder.ToString();
	}

	private static string ResolveTitleName(uint titleId, string titleName)
	{
