...
public sealed class NotifySendCommand : AsyncCommand<NotifySendCommand.Settings> {
    ...
    public override async Task<int> ExecuteAsync(CommandContext context, Settings settings) {
        ...
        if (string.IsNullOrWhiteSpace(message)) {
            return WriteValidationFailure(settings.Json, "Notification text is required. Use `rgh notify \"text\" 14` or `--message`.", "NOTIFY_MESSAGE_REQUIRED");
        }

        if (!NotificationTextPolicy.TryPrepareNotificationMessage(message, out string normalizedMessage, out string? messageError)) {
            return WriteValidationFailure(settings.Json, messageError ?? "Invalid notification text.", "NOTIFY_MESSAGE_INVALID");
        }

        if (!string.IsNullOrWhiteSpace(settings.Icon) && (!string.IsNullOrWhiteSpace(settings.Logo) || !string.IsNullOrWhiteSpace(settings.LogoArgument))) {
            return WriteValidationFailure(settings.Json, "Choose either --icon or a logo value (--logo or positional), not both.", "NOTIFY_LOGO_CONFLICT");
        }
        ...
        if (!NotifyHelpers.TryResolveLogo(settings.Icon, logoValue, out int logo, out string? error)) {
            return WriteValidationFailure(settings.Json, error ?? "Invalid notify options.", "NOTIFY_LOGO_INVALID");
        }

        if (!NotifyHelpers.TryResolvePosition(settings.Position, out int position, out error)) {
            return WriteValidationFailure(settings.Json, error ?? "Invalid notify options.", "NOTIFY_POSITION_INVALID");
        }
    ...
    private static int WriteValidationFailure(bool json, string message, string code) {
        if (json) {
            CliOutput.EmitJsonError(new CliErrorEnvelope(
                ...
                message,
                code,
                new[] { "Correct the notification options and retry." }));
        }
        else {
...
public sealed class NotifyIconsAddCommand : Command<NotifyIconsAddCommand.Settings> {
    ...
    public override int Execute(CommandContext context, Settings settings) {
        ...
        if (!NotifyHelpers.TryResolveLogo(null, settings.Logo, out int logo, out _)) {
            AnsiConsole.MarkupLine("[red]Invalid --logo.[/]");
            return 1;
        }
