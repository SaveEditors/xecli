...
public class OriginalXboxCompatibilityInstallCommand : Command<OriginalXboxCompatibilityInstallCommand.Settings> {
    ...
    public override int Execute(CommandContext context, Settings settings) {
        ...
        if (!validation.Successful) {
            OperationFeedback.WriteFailure("Original Xbox compatibility install", validation.Message ?? "Validation failed.");
            return 1;
        }
        ...
        try {
            ...
            // Keep intentionally thrown validation errors
            if (ex is InvalidOperationException validationError &&
                (validationError.Message.Contains("HddX was not detected") ||
                 validationError.Message.Contains("No writable Hdd1/Usb0/Usb1/Usb2") ||
                 validationError.Message.Contains("Unknown Original Xbox compatibility set"))) {
                OperationFeedback.WriteFailure("Original Xbox compatibility install", validationError.Message);
            }
            else {
                // Sanitize raw exceptions from download/IO/FTP operations
                string message = ex.Message.ToLowerInvariant();
                if (message.Contains("download") || message.Contains("http") || ex is HttpRequestException) {
                    OperationFeedback.WriteFailure("Original Xbox compatibility install", "Download failed. Check your network connection and try again.");
                }
                else {
                    OperationFeedback.WriteFailure("Original Xbox compatibility install", "Install failed. Check available storage and try again.");
                }
            }
            return 1;
        }
