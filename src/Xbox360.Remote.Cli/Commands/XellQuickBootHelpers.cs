...
internal static class XellQuickBootHelpers
{
	...
	public static string DescribeBootMode(XellCommandSettings settings)
	{
		if (settings.ForceXell)
		{
			return "forced direct XeLL reboot";
		}
		if (IsEnabled(settings))
		{
			return TargetsFlash(settings)
				? "QuickBoot XeLL launcher targeting flash"
				: "QuickBoot XeLL launcher targeting the resolved helper";
		}
		if (!string.IsNullOrWhiteSpace(settings.StageLauncherPath) && !string.IsNullOrWhiteSpace(settings.StageXellBinPath))
		{
			return "staged helper and xell.bin";
		}
		if (!string.IsNullOrWhiteSpace(settings.StageLauncherPath))
		{
			return "staged helper";
		}
		if (!string.IsNullOrWhiteSpace(settings.StageXellBinPath))
		{
			return "staged xell.bin beside the resolved helper";
		}
		if (!string.IsNullOrWhiteSpace(settings.LauncherPath))
		{
			return "specified launcher";
		}
		return "auto-resolve XellLaunch, then tray-assisted direct XeLL fallback";
	}
	...
	private static async Task BuildPackageAsync(string outputPath, string displayName, string description, string configPath, CancellationToken cancellationToken)
	{
		...
		{
			string text3 = string.Join(Environment.NewLine, new string?[] { stdOut?.Trim(), stdErr?.Trim() }.Where((string? value) => !string.IsNullOrWhiteSpace(value)).Select(value => value!));
...
if (-not $session.AddFile($ConfigFile, 'config.ini')) {
...
$package = [X360.STFS.STFSPackage]::new($session, $signing, $OutputFile, $log)
...
if (-not $package.CloseIO()) {
    throw 'The XeLL shortcut package stream did not close successfully.'
}
""";
}
