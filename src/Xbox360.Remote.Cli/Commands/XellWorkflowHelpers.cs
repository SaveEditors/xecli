...
internal static class XellWorkflowHelpers
{
	...
	private static async Task<XellHttpEndpoint> AwaitBootedXellAsync((string Ip, int Port, int TimeoutMs) target, XellCommandSettings settings, XellBootRequest? xellBootRequest, CancellationToken cancellationToken)
	{
		...
		if (!settings.Json && xellCustomStatus != null)
		{
	...
	public static void ConfirmFirstXellLaunch(string ip, string actionDescription, XellCommandSettings settings)
	{
		...
		{
			throw new CliErrorException(new CliErrorEnvelope(
				"XeLL launch confirmation required",
				"XeLL was not launched because explicit confirmation is required.",
				"CONFIRMATION_REQUIRED",
				new[]
				{
					"Review the target console state, then rerun with --yes to confirm the XeLL launch.",
					"Run the command interactively to approve the launch prompt, or boot XeLL manually and rerun the command."
				}));
		}
	...
	public static void WriteDryRunPreview(string commandName, string outputPath, string verificationMode, string bootMode, bool json)
	{
		if (json)
		{
		...
		AnsiConsole.MarkupLine("[grey]Dry run:[/] [cyan]no target resolution, connection, or session setup[/]");
		AnsiConsole.MarkupLine("[grey]Planned output path:[/] [cyan]" + Markup.Escape(outputPath) + "[/]");
		AnsiConsole.MarkupLine("[grey]Verification mode:[/] [white]" + Markup.Escape(verificationMode) + "[/]");
		AnsiConsole.MarkupLine("[grey]Boot mode:[/] [white]" + Markup.Escape(bootMode) + "[/]");
	}
