...
internal static class HomebrewConsoleInstallService {
    ...
    public static async Task<HomebrewConsoleInstallResult> InstallAsync(
        ...
        if (!settings.AutoConfirm) {
            ...
            if (!ConfirmationHelpers.TryConfirm(
                    "Homebrew console install",
                    "Continue with the console install?",
        ...
        try {
            ...
            if (Directory.Exists(pluginRoot)) {
                await UploadDirectoryAsync(client, ip, port, user, pass, timeout, pluginRoot, $"/{device.RootName}/Plugins", "Upload user-supplied plugins", cancellationToken);
            ...
            if (iniMode == HomebrewLaunchIniMode.Generated) {
                ...
                string text = HomebrewPackageService.CreateLaunchIniText(device.LaunchAlias, pluginsUploaded);
    ...
    public static void RenderInstallResult(HomebrewConsoleInstallResult result) {
        ...
        if (result.PluginsUploaded)
            AnsiConsole.MarkupLine($"[grey]User-supplied plugins copied to[/] [springgreen3_1]/{Markup.Escape(result.DeviceRoot)}/Plugins[/]");

        string iniStatus = result.LaunchIniMode switch {
            HomebrewLaunchIniMode.Generated => "Generated XeCLI launch.ini",
            HomebrewLaunchIniMode.Merge when result.PluginsUploaded => "Updated existing launch.ini plugin entries",
            HomebrewLaunchIniMode.Merge => "launch.ini left unchanged; no user-supplied console plugins were found",
    ...
    private static HomebrewConsoleDevice ResolveDevice(string? requestedDevice, IReadOnlyList<HomebrewConsoleDevice> devices, bool autoConfirm) {
        ...
        if (autoConfirm || !ConfirmationHelpers.CanPromptForConfirmation()) {
    ...
    private static HomebrewLaunchIniMode ResolveLaunchIniMode(string? requestedMode, bool autoConfirm) {
        if (!string.IsNullOrWhiteSpace(requestedMode)) {
            if (TryParseIniMode(requestedMode, out HomebrewLaunchIniMode parsed))
                return parsed;
            throw new InvalidOperationException("Invalid --ini-mode. Expected generated, create, new, merge, modify, skip, none, or download.");
        }
        ...
        table.AddRow("[white]1[/]", "[springgreen3_1]generated[/]", "[grey]Write a new XeCLI launch.ini. Plugin entries are included only when user-supplied plugins are present.[/]");
        table.AddRow("[white]2[/]", "[springgreen3_1]merge[/]", "[grey]Keep the existing launch.ini and add/update plugin entries only when user-supplied plugins are present.[/]");
    ...
    internal static bool TryParseIniMode(string value, out HomebrewLaunchIniMode mode) {
    ...
    private static string DescribeIniMode(HomebrewLaunchIniMode mode) {
        return mode switch {
            ...
            HomebrewLaunchIniMode.Merge => "Modify launch.ini only when user-supplied plugins are present",
    ...
    private static async Task<(bool Written, bool BackedUp)> MergeLaunchIniAsync(
        ...
        string launchAlias,
        CancellationToken cancellationToken) {
        IReadOnlyList<(int Slot, string Path)> pluginPaths = HomebrewPackageService.GetConsolePluginPaths(launchAlias);

        bool exists = await client.FileExists(iniPath);
        if (!exists) {
            StringBuilder builder = new StringBuilder();
            builder.AppendLine("[Plugins]");
            foreach ((int slot, string path) in pluginPaths)
        ...
        foreach ((_, string pluginPath) in pluginPaths) {
    ...
    private static void RefuseHeadlessConsoleInstall() {
        ...
        OperationFeedback.WriteFailure("Console install blocked", detail);
