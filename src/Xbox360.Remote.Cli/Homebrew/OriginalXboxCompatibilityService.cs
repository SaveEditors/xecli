...
internal static class OriginalXboxCompatibilityService {
    ...
    public static async Task<OriginalXboxCompatibilityInstallResult> InstallToHostAsync(
        ...
        try {
            if (!autoConfirm && !Console.IsInputRedirected) {
                ...
                if (!AnsiConsole.Confirm("Continue with the Original Xbox compatibility staging?", true))
    ...
    public static async Task<OriginalXboxCompatibilityInstallResult> InstallToConsoleAsync(
        ...
        if (!settings.AutoConfirm && !Console.IsInputRedirected) {
            ...
            AnsiConsole.Write(summary);
            if (!AnsiConsole.Confirm("Continue with the Original Xbox compatibility console install?", true))
