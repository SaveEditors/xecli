...
namespace NoDev.Common.IO
{
    public sealed class MultiFileIO : EndianIO
    {
        ...
        private Stream _currentStream = Stream.Null;
        ...
        public override int Read(byte[] buffer, int index, int count)
        {
            ...
            while (totalRead < count)
            {
                ...
                {
                    if (_currentFileIndex + 1 >= _io.Length)
                        return totalRead;
                    OpenFile(_currentFileIndex + 1);
                }

                int requested = Math.Min(count - totalRead, BytesLeftInCurrentFile);
                int read = _currentStream.Read(buffer, index + totalRead, requested);
                if (read == 0)
                    return totalRead;
                totalRead += read;
            }
