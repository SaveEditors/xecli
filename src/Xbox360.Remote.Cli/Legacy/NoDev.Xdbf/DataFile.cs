...
namespace NoDev.Xdbf
{
    ...
    {
        private EndianIO _io = null!;
        private readonly string _fileName = string.Empty;

        private readonly List<DataFileRecord> _records = new List<DataFileRecord>();
        private readonly List<FreeRecord> _freeRecords = new List<FreeRecord>();

        private readonly Dictionary<Namespace, SyncRecord> _syncRecords = new Dictionary<Namespace, SyncRecord>();
        ...
        {
            DataFileRecord record = this.GetRecord(ns, id)
        ...
        public void ExtractRecord(Namespace ns, ulong id, string filename)
        {
            DataFileRecord record = this.GetRecord(ns, id)
                ?? throw new XdbfException(string.Format("Record not found in data file (0x{0:X2}, 0x{1:X16}).", (ushort)ns, id));
            this.ExtractRecord(record, filename);
        }
