...
- a healthy notification-capable JRPC2 path on the console
...
That keeps users away from raw command framing while matching the same message-and-icon model used by other notification-capable tools.
...
XeCLI wraps the practical JRPC2 notification workflow: send message text plus a numeric icon ID to the console's notification path.
...
- message text
- icon/logo ID
- working JRPC2 notification call path
